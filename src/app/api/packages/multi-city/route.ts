...
function logInfo(message: string, data?: Record<string, unknown>) {
  console.log(`[MultiCityPackages] ${message}`, data ? JSON.stringify(data, null, 2) : '');
}
...
function logError(message: string, error: unknown, context?: Record<string, unknown>) {
  ...
  console.error(`[MultiCityPackages] ${message}`, {
    ...errorDetails,
    ...context,
  });
}
...
function logWarning(message: string, data?: Record<string, unknown>) {
  console.warn(`[MultiCityPackages] ${message}`, data ? JSON.stringify(data, null, 2) : '');
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logInfo('Fetching multi-city packages', {
    ...
    let packagesQuery = `
      SELECT DISTINCT
        ...
        p.per_person_price, p.fixed_price, p.pricing_mode, p.currency, 
        p.total_nights, p.total_cities, p.status
    ...
    for (const pkg of packagesResult.rows) {
      try {
        ...
        const packageDetails: PackageWithDetails = {
          ...
          per_person_price: pkg.per_person_price ?? null,
          fixed_price: pkg.fixed_price ?? null,
          pricing_mode: pkg.pricing_mode || null,
        };
