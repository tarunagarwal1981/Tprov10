...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch all published multi-city hotel packages
    const packagesResult = await query<any>(
      `SELECT 
        id, title, destination_region, operator_id, base_price, 
        adult_price, currency, total_nights, total_cities, status
    ...
    console.error('Error fetching multi-city hotel packages:', error);
    return NextResponse.json(
      { error: 'Failed to fetch multi-city hotel packages', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
