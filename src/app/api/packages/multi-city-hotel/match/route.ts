...
/**
 * POST /api/packages/multi-city-hotel/match
 * Get multi-city hotel packages with exact matching (sequence + nights)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const citiesResult = await query<any>(
      `SELECT package_id, name, nights, display_order
    ...
    console.error('Error matching multi-city hotel packages:', error);
    return NextResponse.json(
