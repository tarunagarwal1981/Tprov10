 ...
 * Get purchased leads with aggregated data for management table
 * Shows only leads that were purchased from marketplace (purchased_from_marketplace = true)
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Build WHERE clause
    // Filter by agent_id AND purchased_from_marketplace = true
    const whereConditions: string[] = [
      'l.agent_id::text = $1',
      'l.purchased_from_marketplace = true'
    ];
    const queryParams: any[] = [userId];
    let paramIndex = 2;

    console.log('[Leads Purchased] Query params:', { userId, status, search, page, limit, sortBy, sortOrder });
    ...
    console.log('[Leads Purchased] Executing query with params:', queryParams);
    const result = await query<LeadWithAggregates>(sqlQuery, queryParams);
    console.log('[Leads Purchased] Result count:', result.rows.length);
    if (result.rows.length > 0) {
      console.log('[Leads Purchased] Sample lead IDs:', result.rows.slice(0, 3).map(r => r.id));
    }
    ...
    if (missingLeadsCount > 0) {
      console.log(`[Leads Purchased] Found ${missingLeadsCount} purchased leads without entries in leads table. Syncing...`);
      ...
      for (const missingLead of missingLeads.rows) {
        try {
          ...
          console.log(`[Leads Purchased] Created lead for purchase: ${missingLead.lead_id}`);
        } catch (error: any) {
          console.error(`[Leads Purchased] Failed to create lead for ${missingLead.lead_id}:`, error.message);
        }
      }

      // Re-query after syncing
      const resultAfterSync = await query<LeadWithAggregates>(sqlQuery, queryParams);
      console.log('[Leads Purchased] Result count after sync:', resultAfterSync.rows.length);
    ...
    console.log('[Leads Purchased] Total leads count:', total);
    ...
    console.error('Error fetching purchased leads for management:', error);
